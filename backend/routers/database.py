...
def reset_database(session: Session = Depends(get_session)):
    ...
    try:
        ...
        # Delete borrows (references book_copy and user)
        borrows = session.exec(select(Borrow)).all()
        for borrow in borrows:
            session.delete(borrow)
        
        # Delete donations (references book_copy and user)  
        donations = session.exec(select(Donation)).all()
        for donation in donations:
            session.delete(donation)
        
        # Delete book copies (references book and user as donor)
        book_copies = session.exec(select(BookCopy)).all()
        for book_copy in book_copies:
            session.delete(book_copy)
        
        # Delete books (references category)
        books = session.exec(select(Book)).all()
        for book in books:
            session.delete(book)
        
        # Delete categories (no dependencies)
        categories = session.exec(select(Category)).all()
        for category in categories:
            session.delete(category)
        
        # Delete users (no dependencies, but referenced by book_copies as donors)
        users = session.exec(select(User)).all()
        for user in users:
            session.delete(user)
        ...
        return {
            ...
            "status": "সমস্ত ডেটা মুছে ফেলা হয়েছে"
        }
